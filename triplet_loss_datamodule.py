...
class ZeroShotDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_data = TripletDataset(plant_types=self.train_plants, split='train', transform = True)
            self.val_data = TripletDataset(plant_types=self.train_plants, split= 'val', transform = False)
        ...
        if stage == 'test' or stage is None:
            ...
            self.test_data_full = TripletDataset(plant_types=self.test_plants, split = "test", transform = False)
            self.test_data_per_plant = {plant_name:TripletDataset(plant_types=[plant_name], split = "test", transform = False) for plant_name in self.test_plants}
