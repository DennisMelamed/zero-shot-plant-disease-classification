...
class TripletDataset(Dataset):
    ...
    def get_paths_and_full_classes(self):
        ...
        unhealthy_images = {plant:[] for plant in self.all_plants}
        for plant in self.all_plants:
            ...
            for disease in diseases:
                ...
                for image in images:
                    ...
                    if dataset[-1]["healthy"] == 1:
                        ...
                        unhealthy_images[dataset[-1]['plant']].append(dataset[-1]['path'])

        self.dataset = dataset
        self.healthy_images = healthy_images
        self.unhealthy_images = unhealthy_images
    ...
    def __getitem__(self, idx):
        ...
        healthy_plant_type = np.random.choice(self.all_plants)
        unhealthy_plant_type = np.random.choice(self.all_plants)
        
        healthy_image_fn = self.healthy_images[healthy_plant_type][np.random.randint(len(self.healthy_images[healthy_plant_type]))]
        unhealthy_image_fn = self.unhealthy_images[unhealthy_plant_type][np.random.randint(len(self.unhealthy_images[unhealthy_plant_type]))]
        unknown_image_fn = self.dataset[idx]['path']

        healthy_image = self.default_loader(healthy_image_fn)
        unhealthy_image = self.default_loader(unhealthy_image_fn)
        ...
        if self.transform is not None:
            ...
            unhealthy_image = self.transform(unhealthy_image)
            unknown_image = self.transform(unknown_image)

        healthy_tensor = self.every_im_transform(healthy_image)
        unhealthy_tensor = self.every_im_transform(unhealthy_image)
        unknown_tensor = self.every_im_transform(unknown_image)
        
        if label == 1:
            positive_tensor = healthy_tensor
            negative_tensor = unhealthy_tensor
        else:
            positive_tensor = unhealthy_tensor
            negative_tensor = healthy_tensor

        weight = self.weights[plant_name]['healthy' if label==1 else 'unhealthy']
        
        return {"positive image": positive_tensor,
                "negative image": negative_tensor,
